...
var SWAGGER_METHODS = ['get', 'post', 'put', 'delete', 'patch'],
    SWAGGER_VERSION = '1.0';
...
swagger.createResource = function(path, options) {
  ...
  this.server.get(this.discoveryUrl + path, function(req, res) {
    var result = self._createResponse(req);
    result.basePath = self.basePath || 'http://' + req.headers.host;
